use std::cmp::min;
pub fn levenshtien_distance(s1: &str, s2: &str) -> usize {
    ...
    let mut distances = vec![0; c1.len() + 1];
    let mut new_distances = Vec::with_capacity(distances.len());

    for (index2, char2) in c2.iter().enumerate() {
        new_distances.push(index2 + 1);
